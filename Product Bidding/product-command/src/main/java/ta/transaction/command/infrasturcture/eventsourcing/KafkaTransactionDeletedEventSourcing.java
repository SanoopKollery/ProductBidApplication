...
@Component
@Log4j2
public class KafkaTransactionDeletedEventSourcing {

    @Autowired
    private KafkaTemplate<String, String> kafkaTemplate;

    @Value(value = "${message.topic.deleteProduct}")
    private String topicName;

    public ProductDeleteEvent publicDeleteProductEvent(Product product) throws JsonProcessingException {
        ObjectWriter objectWriter = new ObjectMapper().writer().withDefaultPrettyPrinter();
        val json = objectWriter.writeValueAsString(product);
        log.info("Send json '{}' to topic {}", json, topicName);
        kafkaTemplate.send(topicName, json);
        return ProductDeleteEvent.builder().message("Successfully deleted !!").build();
    }
