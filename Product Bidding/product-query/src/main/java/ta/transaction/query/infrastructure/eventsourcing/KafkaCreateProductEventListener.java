...
@Log4j2
@Component
public class KafkaCreateProductEventListener {
    ...
    @KafkaListener(topics = "${message.topic.createProduct}")
    public void listen(ConsumerRecord<String, String> stringStringConsumerRecord) throws Exception {
        ...
        findProductService.createProduct(product);
        log.info("Insert trxn {} in reader database", product);
        latch.countDown();
    }
}
